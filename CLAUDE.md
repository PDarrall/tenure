...
- `apps/web` — React shell. Depends on engine only. Styled to the design canvas ("Tenure"), exported under `design/v2/`: a phone column of Home, Squad, Tactics, Fixtures and Career, one Continue button, tokens in `apps/web/src/styles.css`. `DESIGN.md` "Interface" holds the rules.
